...
A lightweight application made with Swing, the goal of this program is to provide a tool for a very hands on approach to recording the anime a person has watched. In addition to the list like functionality, the program includes individual pages for each show, a statistics page, UI customization, and more.
...
* [Usage](#usage)
  ...
  + [Adding and Editing Anime](#adding-and-editing-anime)
...
Click [here](https://github.com/Blueredemption/AnimeList/releases/tag/1.0.4) to download the program. Once competed, move the .zip to the directory of choice and extract the files. Create a shortcut if you would like, but the executable file must stay with the resource files to function correctly. This program requires JRE 1.8 or newer to run.
...
To add an anime, click the "+" / "New Anime" button on the navigation tab. Once clicked the program will create a new anime, add it to the list, and bring you directly to the editing page. Each button is self-explanatory. Once clicked, in most cases the user is prompted to fill the field through a combination of dropdowns, buttons, and text fields that appear in the bottom right portion of the window. In the case of adding an image, a file selector window appears. 
...
1) If you are using this program for the first time and you don't know the dates of when you started or finished the anime you are entering, you should try to enter them in order of when you watched them, starting with the first. The algorithm for sorting by date started sorts by when they are added for anime that don't have start dates, so keep that in mind of you want more accurate ordering.

2) It is recommended that you back up the "Anime Objects" file if you want to make sure you don't lose all of your hard work from an *act of god*. This is where all of your anime entries are stored.

3) When selecting images, it is best to move them into the "Images/Anime" file before selecting them in the program. This way the image is less likely to be moved or deleted unintentionally, which will break the image inside the program.
